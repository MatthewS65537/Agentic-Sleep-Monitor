...
from ZeroShotWriter import *

ell.init(store='./logdir', autocommit=True)

def aggregator(model, responses, template, temperature=0.5, max_tokens=2048):    
    @ell.simple(
        model=model.split("/")[-1],
...
class MoAWriter:
    ...
    def run(self):
        ...
        responses = [zero_shot_writer(self.model, self.data, self.template) for _ in range(self.num_agents)]
