...
def plot_moa_performance(models, names, output_file, title, lower_y=5, upper_y=10):
    accuracy = []
    clarity = []
    relevancy = []
    style = []
    ...
    for model in models:
        ...
        if "1Pass" not in model:
            accuracy.append(np.mean(accuracy_[2:-2]))
            clarity.append(np.mean(clarity_[2:-2]))
            relevancy.append(np.mean(relevancy_[2:-2]))
            style.append(np.mean(style_[2:-2]))
            quality.append(np.mean(quality_[2:-2]))
        avg.append(np.mean(avg_[2:-2]))
